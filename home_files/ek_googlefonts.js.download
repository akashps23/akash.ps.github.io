...
Google Font Auto-Loader v3
This script will load Google Fonts for any element in your HTML page
without adding any links to individual fonts.
...
/*
    function sizer(){
        ...
        [...document.querySelectorAll('div,img')].forEach((ele) => {
            ...
            let pstyle = window.getComputedStyle(ele.parentNode);

            let width = parseInt(style.getPropertyValue('width').split('px').join(''));
            let height = parseInt(style.getPropertyValue('height').split('px').join(''));
            let pwidth = parseInt(pstyle.getPropertyValue('width').split('px').join(''));
            let pheight = parseInt(pstyle.getPropertyValue('height').split('px').join(''));

            let top = parseInt(style.getPropertyValue('top').split('px').join(''));
            let left = parseInt(style.getPropertyValue('left').split('px').join(''));
            let ptop = parseInt(pstyle.getPropertyValue('top').split('px').join(''));
            let pleft = parseInt(pstyle.getPropertyValue('left').split('px').join(''));

            ele.style['width'] = (width/pwidth)*100+'%';
            ele.style['height'] = (height/pheight)*pheight+'px';
            ele.style['top'] = (top/pheight)*100+'%';
            ele.style['left'] = (left/pwidth)*100+'%';
    ...
    window.onresize = () => {
        clearTimeout(sizeActive);
        sizeActive = setTimeout( ()=>{ 
            sizer();
        }, 200);
    };
